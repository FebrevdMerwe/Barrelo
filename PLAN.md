...
- **Bed vocabulary — partially confirmed.** `"SingleOuter"` → `Outer`, `"SingleInner"` → `Inner`, `"Triple"` → `Triple`, `"Outside"` (multiplier 0) → `Miss` are confirmed from real captures. `"Double"` → `Double`, `"DoubleBull"` → `InnerBull` (50), `"Bull"` → `OuterBull` (25) are best-guess pending a real capture of those throws; the adapter throws loudly on any unrecognized bed string rather than silently mis-scoring, so a wrong guess surfaces immediately in logs.
