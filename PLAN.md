...
- **Delivery, mirroring the existing DLL post-build copy (below).** A game project may include a `ui/` folder (e.g. `src/Games/Darts.Games.X01/ui/render.js`); an optional post-build target copies it into `Darts.Api/wwwroot/plugins/{gameId}/`, parallel to the DLL copy into `Darts.Api/plugins/{gameId}/`. Pure MSBuild copy wiring — no project reference changes, so the plugin still references only `Darts.GameSdk`.
...
- Each game project's optional `ui/render.js` + its post-build copy target (e.g. `Darts.Games.X01.csproj`) — the per-game board UI convention.
