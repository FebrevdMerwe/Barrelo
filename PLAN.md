...
> **Post-v1 update — this played out, then got replaced.** Out-of-process hosting *was* built later, exactly as the paragraph below predicted: a `RemoteGameLoader` contributing `RemoteGameFactory` instances into the same catalog, proxying `IGame` over HTTP/JSON to a spawned process, with zero changes to `GameCatalog`, `GameSessionManager`, or the API layer. The additive-loader claim held.
>
> It has since been **removed** in favour of **client-owned games**, where a game is purely a browser app and the host keeps only an append-only log of *visits* without interpreting it (`ClientGameLoader` → `ClientGameFactory` → `ClientOwnedGame`, still just another `IGameFactory` in the same catalog). The reason wasn't that out-of-process hosting failed — it worked — but that it made every non-.NET game cost *two* deliverables, a headless rules server and a browser UI, plus port allocation, health polling, process lifetime, and a crash-abort path. Auditing what the host actually did with a game's state found almost nothing: no persistence at all, just a session leaderboard, the single-active-match slot, and scoreboard chrome. Everything the host genuinely needed survives as a one-shot result message from the client, so the process half was pure cost.
>
> Two consequences worth recording. **Turn attribution stays out of the log** — a visit records darts and a boundary, never a player id, because who throws next is a rules decision (elimination games skip dead players; "hit a bull, throw again" breaks round-robin), and stamping it host-side would have quietly put the host back in the rules business. And **replay determinism became a real contract**: every screen derives its own state from the same log, so a stray `Math.random()` desyncs the TV from the tablet. That failure is invisible in practice, so clients report a hash of the log they replayed alongside a hash of the state they derived, and the host warns when the same input yields different output.
...
### AutoDarts wire format (confirmed)

Confirmed against a live local board manager (`ws://localhost:3180/api/events`, captured samples in `EVENTS.txt`) — no longer speculative:
- **Local vs cloud — resolved.** The local board-manager stream at `ws://127.0.0.1:3180/api/events` exposes real-time per-throw events directly; no cloud relay or auth/token flow is needed. The "single device, no cloud" constraint (Deployment) holds as-is.
- **Event schema & cardinality — resolved.** Every message is `{"type": "state", "data": {...}}`. AutoDarts confirmed to emit one fused throw per physical dart (no per-camera duplication). `data.event` is the real discriminator: `"Throw detected"` carries the *cumulative* darts of the current visit (`data.throws`, up to 3, growing by one per dart — not one message per dart, so the adapter diffs against the last-seen array), `"Takeout started"` signals the hand entering the board (no new dart), and `"Takeout finished"` (data.numThrows resets to 0) is the turn-boundary signal mapped to `DetectionEventType.EndOfTurn`. Each dart is `segment.name`/`segment.number`/`segment.bed`/`segment.multiplier` + `coords.x`/`coords.y` — a bed/ring name (e.g. `"SingleOuter"`, `"Triple"`, `"Outside"` for miss) rather than OpenDartboard's `"D20"`-style token, confirming the adapter needs its own bed-name mapping distinct from OpenDartboard's notation parsing. `coords.x`/`coords.y` empirically match `BoardPosition`'s own normalized convention (same origin/axes/scale/orientation) closely enough to pass through untransformed.
- **Bed vocabulary — confirmed, including bull.** `"SingleOuter"` → `OuterSingle`, `"SingleInner"` → `InnerSingle`, `"Triple"` → `Triple`, `"Outside"` (multiplier 0) → `Miss` are confirmed from real captures. The bullseye is *not* a distinct bed name — a real capture confirmed it arrives as `{ number: 25, bed: "Single" | "Double", multiplier: 1 | 2 }`, the same shape as any other segment. This is why the internal `Ring` model dropped its dedicated `InnerBull`/`OuterBull` cases in favor of `Ring.Single`/`Ring.Double` at segment 25 (see `DartScoring.IsBull`) — AutoDarts itself doesn't think of the bull as a special ring, so neither does Barrelo. `("Double", 1-20)` for wedge doubles remains the one bed/number pair still unconfirmed against a real capture; the adapter throws loudly on any unrecognized bed/number pair rather than silently mis-scoring, so a wrong guess surfaces immediately in logs.
- **Corrections — deferred.** No correction/retract event has been captured yet. v1's adapter ignores any `data.event` it doesn't recognize (logged at Debug); manual correction stays the platform's own `POST /api/detection/manual-throw` path, as originally planned.
...
- **`AutoDartsDetectionSource.cs`** — connects to the AutoDarts local board-manager event stream, maps each event to `DetectedThrow`/`DetectionEvent`, simple exponential-backoff reconnect (see AutoDarts wire format above). Config: `Detection:AutoDarts:Url` (defaults to `ws://localhost:3180/api/events`).
...
- Bust: `remaining < 0`, or `remaining == 1` under double-out, or (`remaining == 0` and the finishing dart wasn't a valid double under double-out) → revert the visit's score change, end turn. **Valid double-out finisher = `Ring.Double`** — since the inner bull (50) is itself `Ring.Double` at segment 25 (see the `Ring` model note above), this single check already accepts a double-bull finish with no bull-specific branch. `Ring.Single` at segment 25 (the 25-point outer bull) is not a double and correctly busts a checkout dart under double-out.
...
**Phase 3 — AutoDarts adapter.** `AutoDartsDetectionSource`, reconnect logic, `Detection:Mode` switch. **Wire format confirmed** (see AutoDarts wire format above) against a live local board manager and real captured sample events (`EVENTS.txt`) — local stream (no cloud/auth), event schema and turn-boundary signal, one-fused-throw-per-dart, and the bull bed vocabulary (`Single`/`Double` at segment 25, not a distinct bull bed name) all resolved; wedge doubles (`"Double"` at segment 1-20) remain the one bed/number pair still a best guess pending a real capture, with the adapter failing loudly on any unrecognized bed/number pair rather than silently mis-scoring. Automated replay-test coverage against a fake local WS server (`Infrastructure.IntegrationTests`, mirroring `FakeGameServer.cs`'s pattern) is deferred, not yet built.
...
- Phase 3: the wire-format gate (local vs cloud, event schema, fusion, corrections) was resolved with real captured samples (`EVENTS.txt`), not assumed. Verified live against the user's real board manager rather than a replayed-frame test server (automated replay-test coverage deferred, see Phase 3 above) — confirm a full visit (including a dropped-then-reconnected WebSocket) updates `GameStateSnapshot` correctly end to end.
